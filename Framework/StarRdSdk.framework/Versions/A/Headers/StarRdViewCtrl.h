...
/// 17k章节目录
+ (void)openCatalogViewCtrlWith17KBookID:(NSInteger)bID chapterID:(NSInteger)cID;

/// 转码章节目录
+ (void)openCatalogViewCtrlWithBookID:(NSInteger)bID cataLogURL:(NSString *)cataLogURL chapterURL:(NSString *)chapterURL;

/// 阅读页
+ (void)openReaderWithBookID:(NSInteger)bID chapterID:(NSInteger)cID;

/// 阅读页 17k传字典
+ (void)openReaderWithDic:(NSDictionary *)dic isInShelf:(BOOL)isInShelf;

/////转码阅读页-传内容
+ (void)openSubscritionReaderWithDict:(NSDictionary *)dict;

/////转码阅读页-传内容+搜索转码类型
+ (void)openSearchSubscritionReaderWithDict:(NSDictionary *)dict searchKey:(NSString *)searchKey searchSub:(NSInteger)searchType;

/////转码阅读页-传地址
+ (void)openSubscritionReaderWithURL:(NSString *)url bookId:(NSInteger)bookId;
