...
typedef void (^MCRequestCompletionBlock)(__kindof StarRdMCBaseRequest *request);

@interface StarRdMCBaseRequest : NSObject
...
/// 请求回调代理
@property (nonatomic, weak, nullable) id<StarRdMCRequestDelegate> delegate;
...
/// 从请求队列中移除, 并取消请求, 该方法会导致回调block置空
- (void)stop;
